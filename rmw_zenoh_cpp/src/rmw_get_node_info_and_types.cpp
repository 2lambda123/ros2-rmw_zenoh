...
extern "C" {
///==============================================================================
// Return all topic names and types for which a given remote node has
// subscriptions.
rmw_ret_t rmw_get_subscriber_names_and_types_by_node(
  ...
  return node->context->impl->graph_cache.get_entity_names_and_types_by_node(
      liveliness::EntityType::Subscription, allocator, node_name,
      node_namespace, no_demangle, topic_names_and_types);
}
...
rmw_ret_t rmw_get_publisher_names_and_types_by_node(
  ...
  RMW_CHECK_ARGUMENT_FOR_NULL(node->context->impl, RMW_RET_INVALID_ARGUMENT);
  return node->context->impl->graph_cache.get_entity_names_and_types_by_node(
      liveliness::EntityType::Publisher, allocator, node_name, node_namespace,
      no_demangle, topic_names_and_types);
}

///==============================================================================
/// Return all service names and types for which a given remote node has
/// servers.
rmw_ret_t rmw_get_service_names_and_types_by_node(
  ...
  RMW_CHECK_ARGUMENT_FOR_NULL(node->context->impl, RMW_RET_INVALID_ARGUMENT);
  return node->context->impl->graph_cache.get_entity_names_and_types_by_node(
      liveliness::EntityType::Service, allocator, node_name, node_namespace,
      false, service_names_and_types);
}
...
rmw_ret_t rmw_get_client_names_and_types_by_node(
  ...
  RMW_CHECK_ARGUMENT_FOR_NULL(node->context->impl, RMW_RET_INVALID_ARGUMENT);
  return node->context->impl->graph_cache.get_entity_names_and_types_by_node(
      liveliness::EntityType::Client, allocator, node_name, node_namespace,
      false, service_names_and_types);
}
} // extern "C"
