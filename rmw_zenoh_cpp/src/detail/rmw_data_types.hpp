...
struct rmw_node_data_t {
  ...
  // for cases where a node may spin up but does not have any publishers or
  // subscriptions. Liveliness token for the node.
...
struct saved_msg_data {
  explicit saved_msg_data(zc_owned_payload_t p, uint64_t recv_ts,
...
struct rmw_service_data_t {
  ...
  std::unordered_map<unsigned int, std::unique_ptr<z_owned_query_t>>
      id_query_map;
