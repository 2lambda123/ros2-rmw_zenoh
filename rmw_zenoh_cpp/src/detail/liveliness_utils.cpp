...
///=============================================================================
NodeInfo::NodeInfo(std::size_t domain_id, std::string ns, std::string name,
                   std::string enclave)
    : domain_id_(std::move(domain_id)), ns_(std::move(ns)),
      name_(std::move(name)), enclave_(std::move(enclave)) {
  // Do nothing.
}

///=============================================================================
TopicInfo::TopicInfo(std::string name, std::string type, std::string qos)
    : name_(std::move(name)), type_(std::move(type)), qos_(std::move(qos)) {
  // Do nothing.
}
...
std::string subscription_token(size_t domain_id) {
  std::string token =
      std::string(ADMIN_SPACE) + "/" + std::to_string(domain_id) + "/**";
  return token;
}

///=============================================================================
Entity::Entity(EntityType type, NodeInfo node_info,
               std::optional<TopicInfo> topic_info)
    : type_(std::move(type)), node_info_(std::move(node_info)),
   ...
   *
   * Where:
   *  <domainid> - A number set by the user to "partition" graphs.  Roughly
   * equivalent to the domain ID in DDS. <entity> - The type of entity.  This
   * can be one of "NN" for a node, "MP" for a publisher, "MS" for a
   * subscription, "SS" for a service server, or "SC" for a service client.
   *  <namespace> - The ROS namespace for this entity.  If the namespace is
   * absolute, this function will add in an _ for later parsing reasons.
   *  <nodename> - The ROS node name for this entity.
   *
   * For entities with topic infomation, the liveliness token keyexpr have
   * additional fields:
   *
  ...
  token_ss << ADMIN_SPACE << "/" << node_info_.domain_id_ << "/"
           << entity_to_str.at(type_) << ns;
  // An empty namespace from rcl will contain "/" but zenoh does not allow keys
  // with "//". Hence we add an "_" to denote an empty namespace such that
  // splitting the key will always result in 5 parts.
  if (ns == "/") {
    token_ss << "_/";
  } else {
    token_ss << "/";
  ...
  if (topic_info_.has_value()) {
    ...
    // Note: We don't append a leading "/" as we expect the ROS topic name to
    // start with a "/".
    token_ss << topic_info.name_ + "/" + topic_info.type_ + "/" +
                    topic_info.qos_;
  }
...
///=============================================================================
std::optional<Entity> Entity::make(EntityType type, NodeInfo node_info,
                                   std::optional<TopicInfo> topic_info) {
...
std::vector<std::string> split_keyexpr(const std::string &keyexpr,
  ...
  std::size_t idx = 0;
  for (std::string::const_iterator it = keyexpr.begin(); it != keyexpr.end();
       ++it) {
    if (*it == delim) {
...
std::optional<Entity> Entity::make(const std::string &keyexpr) {
  ...
  if (parts[0] != ADMIN_SPACE) {
    RCUTILS_LOG_ERROR_NAMED(
        "rmw_zenoh_cpp", "Received liveliness token with invalid admin space.");
    return std::nullopt;
  }
  ...
  if (entity_type != EntityType::Node) {
    if (parts.size() < 8) {
      RCUTILS_LOG_ERROR_NAMED("rmw_zenoh_cpp",
                              "Received liveliness token for non-node entity "
                              "without required parameters.");
      return std::nullopt;
    }
    topic_info = TopicInfo{"/" + std::move(parts[5]), std::move(parts[6]),
                           std::move(parts[7])};
  }
...
///=============================================================================
EntityType Entity::type() const { return this->type_; }

std::string Entity::node_namespace() const { return this->node_info_.ns_; }

std::string Entity::node_name() const { return this->node_info_.name_; }

std::string Entity::node_enclave() const { return this->node_info_.enclave_; }
...
///=============================================================================
std::string Entity::keyexpr() const { return this->keyexpr_; }

///=============================================================================
bool PublishToken::put(z_owned_session_t *session, const std::string &token) {
  ...
  if (!z_keyexpr_check(&keyexpr)) {
    RCUTILS_SET_ERROR_MSG(
        "invalid keyexpression generation for liveliness publication.");
    return false;
  }
  ...
  if (z_put(z_loan(*session), z_keyexpr(token.c_str()), nullptr, 0, &options) <
      0) {
...
///=============================================================================
bool PublishToken::del(z_owned_session_t *session, const std::string &token) {
  if (!z_session_check(session)) {
    RCUTILS_SET_ERROR_MSG("The zenoh session is invalid.");
    return false;
  }
  ...
  RCUTILS_LOG_WARN_NAMED("rmw_zenoh_cpp", "Sending DELETE on %s",
                         token.c_str());
