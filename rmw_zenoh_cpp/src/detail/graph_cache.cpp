...
TopicStats::TopicStats(std::size_t pub_count, std::size_t sub_count)
    : pub_count_(pub_count), sub_count_(sub_count) {
  // Do nothing.
}

///=============================================================================
TopicData::TopicData(liveliness::TopicInfo info, TopicStats stats)
    : info_(std::move(info)), stats_(std::move(stats)) {}

///=============================================================================
void GraphCache::parse_put(const std::string &keyexpr) {
  std::optional<liveliness::Entity> valid_entity =
      liveliness::Entity::make(keyexpr);
  ...
  auto add_topic_data = [this](const Entity &entity,
                               GraphNode &graph_node) -> void {
    ...
    if (!entity.topic_info().has_value()) {
      // This should not happen as add_topic_data() is called after validating
      // the existence of topic_info.
    ...
    GraphNode::TopicMap &topic_map = entity.type() == EntityType::Publisher
                                         ? graph_node.pubs_
                                         : graph_node.subs_;
    const std::string entity_desc =
        entity.type() == EntityType::Publisher ? "publisher" : "subscription";
    const std::size_t pub_count =
        entity.type() == EntityType::Publisher ? 1 : 0;
    const std::size_t sub_count = !pub_count;
    TopicDataPtr graph_topic_data = std::make_shared<TopicData>(
        topic_info, TopicStats{pub_count, sub_count});
    ...
    if (!insertion.second) {
      // A topic with the same name already exists in the node so we append the
      // type.
      std::pair<GraphNode::TopicDataMap::iterator, bool> type_insertion =
          insertion.first->second.insert(
              std::make_pair(graph_topic_data->info_.type_, graph_topic_data));
      if (!type_insertion.second) {
        // We have another instance of a pub/sub over the same topic and type so
    ...
    // Bookkeeping: Update graph_topics_ which keeps track of topics across all
    // nodes in the graph
    ...
    if (cache_topic_it == graph_topics_.end()) {
      ...
      graph_topics_[topic_info.name_] =
          GraphNode::TopicDataMap{{topic_info.type_, topic_data_ptr}};
    } else {
      // If a TopicData entry for the same type exists in the topic map, update
      // pub/sub counts or else create an new TopicData.
      std::pair<GraphNode::TopicDataMap::iterator, bool> topic_data_insertion =
          cache_topic_it->second.insert(
              std::make_pair(topic_info.type_, nullptr));
    ...
    RCUTILS_LOG_INFO_NAMED(
        ...
        entity_desc.c_str(), topic_info.name_.c_str(), topic_info.type_.c_str(),
        topic_info.qos_.c_str(), graph_node.name_.c_str());
  };
  ...
  // If the namespace did not exist, create it and add the node to the graph and
  // return.
  NamespaceMap::iterator ns_it = graph_.find(entity.node_namespace());
  if (ns_it == graph_.end()) {
    NodeMap node_map = {{entity.node_name(), make_graph_node(entity)}};
    graph_.emplace(
        std::make_pair(entity.node_namespace(), std::move(node_map)));
    RCUTILS_LOG_WARN_NAMED(
        "rmw_zenoh_cpp", "Added node /%s to a new namespace %s in the graph.",
        entity.node_name().c_str(), entity.node_namespace().c_str());
    return;
  }
  ...
  if (node_it == ns_it->second.end()) {
    ns_it->second.insert(
        std::make_pair(entity.node_name(), make_graph_node(entity)));
    RCUTILS_LOG_WARN_NAMED(
        "rmw_zenoh_cpp",
        "Added node /%s to an existing namespace %s in the graph.",
        entity.node_name().c_str(), entity.node_namespace().c_str());
    return;
  }
  ...
  if (entity.type() == EntityType::Node) {
    ...
    // If control reaches here, then we received a duplicate entry for the same
    // node. This could happen when we get() all the liveliness tokens when the
    // node spins up and receive a MP token before an NN one.
    return;
  }
  ...
  if (!entity.topic_info().has_value()) {
    ...
    RCUTILS_LOG_WARN_NAMED("rmw_zenoh_cpp",
                           "Put token %s parsed without extracting topic_info.",
                           keyexpr.c_str());
    return;
  }
...
void GraphCache::parse_del(const std::string &keyexpr) {
  ...
  // Helper lambda to append pub/subs to the GraphNode.
  // We capture by reference to update caches like graph_topics_ if update_cache
  // is true.
  auto remove_topic_data = [&](const Entity &entity, GraphNode &graph_node,
    ...
    // Decrement the relevant counters. If both counters are 0 remove from
    // graph_node.
    ...
    // If the topic does not have any TopicData entries, erase the topic from
    // the map.
    if (topic_data_map.empty()) {
      topic_map.erase(topic_info.name_);
    }

    // Bookkeeping: Update graph_topic_ which keeps track of topics across all
    // nodes in the graph.
    if (update_cache) {
      GraphNode::TopicMap::iterator cache_topic_it =
          graph_topics_.find(topic_info.name_);
      if (cache_topic_it == graph_topics_.end()) {
        ...
        RCUTILS_LOG_ERROR_NAMED(
            "rmw_zenoh_cpp",
            "topic_key %s not found in graph_topics_. Report this.",
            topic_info.name_.c_str());
      } else {
        ...
        if (cache_topic_data_it != cache_topic_it->second.end()) {
          ...
          // If the topic does not have any TopicData entries, erase the topic
          // from the map.
  ...
  if (entity.type() == EntityType::Node) {
    // Node
    // The liveliness tokens to remove pub/subs should be received before the
    // one to remove a node given the reliability QoS for liveliness subs.
    // However, if we find any pubs/subs present in the node below, we should
    // update the count in graph_topics_.
    const GraphNodePtr graph_node = node_it->second;
    if (!graph_node->pubs_.empty() || !graph_node->subs_.empty()) {
      RCUTILS_LOG_WARN_NAMED("rmw_zenoh_cpp",
                             "Received liveliness token to remove node /%s "
                             "from the graph before all pub/subs for this "
                             "node have been removed. Report this issue.",
                             entity.node_name().c_str());
      // TODO(Yadunund): Iterate through the nodes pubs_ and subs_ and decrement
      // topic count in graph_topics_.
    }
    ns_it->second.erase(entity.node_name());
    RCUTILS_LOG_WARN_NAMED("rmw_zenoh_cpp", "Removed node /%s from the graph.",
                           entity.node_name().c_str());
    return;
  }
  ...
  if (!entity.topic_info().has_value()) {
    // Likely an error with parsing the token.
    RCUTILS_LOG_WARN_NAMED("rmw_zenoh_cpp",
                           "Del token %s parsed without extracting TopicData.",
                           keyexpr.c_str());
    return;
  }
...
rmw_ret_t GraphCache::get_node_names(rcutils_string_array_t *node_names,
  ...
  RCUTILS_CHECK_ALLOCATOR_WITH_MSG(allocator,
                                   "get_node_names allocator is not valid",
                                   return RMW_RET_INVALID_ARGUMENT);
  ...
  auto free_node_names = rcpputils::make_scope_exit([node_names]() {
    rcutils_ret_t ret = rcutils_string_array_fini(node_names);
    if (ret != RCUTILS_RET_OK) {
      RCUTILS_LOG_ERROR_NAMED("rmw_zenoh_cpp",
                              "failed to cleanup during error handling: %s",
  ...
  auto free_node_namespaces = rcpputils::make_scope_exit([node_namespaces]() {
  ...
  std::shared_ptr<rcpputils::scope_exit<decltype(free_enclaves_lambda)>>
      free_enclaves{nullptr};
  if (enclaves) {
  ...
  std::size_t j = 0;
  for (NamespaceMap::const_iterator ns_it = graph_.begin();
       ns_it != graph_.end(); ++ns_it) {
    const std::string &ns = ns_it->first;
    for (NodeMap::const_iterator node_it = ns_it->second.begin();
         node_it != ns_it->second.end(); ++node_it) {
      const GraphNodePtr node = node_it->second;
      node_names->data[j] = rcutils_strdup(node->name_.c_str(), *allocator);
      if (!node_names->data[j]) {
        return RMW_RET_BAD_ALLOC;
      }
      node_namespaces->data[j] = rcutils_strdup(ns.c_str(), *allocator);
      if (!node_namespaces->data[j]) {
        return RMW_RET_BAD_ALLOC;
      }
      if (enclaves) {
        enclaves->data[j] = rcutils_strdup(node->enclave_.c_str(), *allocator);
        if (!enclaves->data[j]) {
          return RMW_RET_BAD_ALLOC;
        }
      }
...
// https://github.com/ros2/rmw_cyclonedds/blob/f7f67bdef82f59558366aa6ce94ef9af3c5ab569/rmw_cyclonedds_cpp/src/demangle.cpp#L67
std::string _demangle_if_ros_type(const std::string &dds_type_string) {
  ...
  std::string type_name =
      dds_type_string.substr(start, dds_type_string.length() - 1 - start);
  return type_namespace + type_name;
}

rmw_ret_t fill_names_and_types(const GraphNode::TopicMap &entity_map,
                               rcutils_allocator_t *allocator,
                               rmw_names_and_types_t *names_and_types) {
  const std::size_t entity_size = entity_map.size();
  rmw_ret_t ret =
      rmw_names_and_types_init(names_and_types, entity_size, allocator);
  if (ret != RMW_RET_OK) {
    return ret;
  }
  auto cleanup_names_and_types = rcpputils::make_scope_exit([names_and_types] {
    rmw_ret_t fail_ret = rmw_names_and_types_fini(names_and_types);
    if (fail_ret != RMW_RET_OK) {
      RMW_SAFE_FWRITE_TO_STDERR(
          "failed to cleanup names and types during error handling");
    }
  });
  // Fill topic names and types.
  std::size_t index = 0;
  for (const std::pair<const std::string, GraphNode::TopicDataMap> &item :
       entity_map) {
    names_and_types->names.data[index] =
        rcutils_strdup(item.first.c_str(), *allocator);
    if (!names_and_types->names.data[index]) {
      return RMW_RET_BAD_ALLOC;
    }
    {
      rcutils_ret_t rcutils_ret = rcutils_string_array_init(
          &names_and_types->types[index], item.second.size(), allocator);
      if (RCUTILS_RET_OK != rcutils_ret) {
    ...
    for (const std::pair<const std::string, TopicDataPtr> &type : item.second) {
      char *type_name =
          rcutils_strdup(_demangle_if_ros_type(type.first).c_str(), *allocator);
      if (!type_name) {
...
///=============================================================================
rmw_ret_t GraphCache::count_publishers(const char *topic_name,
                                       size_t *count) const {
  *count = 0;
  std::lock_guard<std::mutex> lock(graph_mutex_);
  if (graph_topics_.count(topic_name) != 0) {
    for (const std::pair<const std::string, TopicDataPtr> &it :
         graph_topics_.at(topic_name)) {
...
///=============================================================================
rmw_ret_t GraphCache::count_subscriptions(const char *topic_name,
                                          size_t *count) const {
  *count = 0;
  std::lock_guard<std::mutex> lock(graph_mutex_);
  if (graph_topics_.count(topic_name) != 0) {
...
rmw_ret_t GraphCache::get_entity_names_and_types_by_node(
    liveliness::EntityType entity_type, rcutils_allocator_t *allocator,
  ...
  if (entity_type == EntityType::Publisher) {
    return fill_names_and_types(node_it->second->pubs_, allocator,
                                names_and_types);
  } else if (entity_type == EntityType::Subscription) {
    return fill_names_and_types(node_it->second->subs_, allocator,
                                names_and_types);
  } else {
    return RMW_RET_OK;
  }
