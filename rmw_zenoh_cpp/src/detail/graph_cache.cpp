...
///=============================================================================
static std::string generate_base_token(const std::string &entity,
                                       size_t domain_id,
...
///=============================================================================
std::string GenerateToken::node(size_t domain_id, const std::string &namespace_,
                                const std::string &name) {
...
///=============================================================================
std::string
GenerateToken::publisher(size_t domain_id, const std::string &node_namespace,
                         const std::string &node_name, const std::string &topic,
                         const std::string &type, const std::string &qos) {
  std::string token =
      generate_base_token("MP", domain_id, node_namespace, node_name);
...
///=============================================================================
PublisherData::PublisherData(const char *topic, const char *node,
                             const char *namespace_, const char *type,
...
///=============================================================================
SubscriptionData::SubscriptionData(const char *topic, const char *node,
                                   const char *namespace_, const char *type,
                                   rcutils_allocator_t *allocator)
    : allocator_(allocator) {
  // TODO(clalancette): Check for error
...
///=============================================================================
uint64_t GraphCache::add_publisher(const char *topic, const char *node,
                                   const char *namespace_, const char *type,
                                   rcutils_allocator_t *allocator) {
  std::lock_guard<std::mutex> lck(publishers_mutex_);
  uint64_t this_handle_id = publishers_handle_id_++;
  publishers_.emplace(std::make_pair(
      this_handle_id, std::make_unique<PublisherData>(topic, node, namespace_,
                                                      type, allocator)));
  return this_handle_id;
}

///=============================================================================
void GraphCache::remove_publisher(uint64_t handle) {
  std::lock_guard<std::mutex> lck(publishers_mutex_);
...
///=============================================================================
uint64_t GraphCache::add_subscription(const char *topic, const char *node,
                                      const char *namespace_, const char *type,
                                      rcutils_allocator_t *allocator) {
  ...
  subscriptions_.emplace(std::make_pair(
      this_handle_id, std::make_unique<SubscriptionData>(
                          topic, node, namespace_, type, allocator)));
  return this_handle_id;
}

///=============================================================================
void GraphCache::remove_subscription(uint64_t handle) {
  std::lock_guard<std::mutex> lck(subscriptions_mutex_);
...
void GraphCache::parse_put(const std::string &keyexpr) {
  ...
  if (entity == "NN") {
    // Node
    RCUTILS_LOG_WARN_NAMED("rmw_zenoh_cpp", "Adding node %s to the graph.",
                           parts.back().c_str());
    ...
    RCUTILS_LOG_ERROR_NAMED(
        "rmw_zenoh_cpp", "Received liveliness token with invalid entity type.");
    return;
  }
}
...
void GraphCache::parse_del(const std::string &keyexpr) {
  ...
  if (entity == "NN") {
    // Node
    RCUTILS_LOG_WARN_NAMED("rmw_zenoh_cpp", "Removing node %s from the graph.",
                           parts.back().c_str());
