...
// Zenoh IDs are LSB-first 128bit unsigned and non-zero integers in hexadecimal
// lowercase.
...
rmw_ret_t zenoh_router_check(z_session_t session) {
  ...
  z_owned_closure_zid_t router_callback =
      z_closure(callback, nullptr /* drop */, &context);
  if (z_info_routers_zid(session, z_move(router_callback))) {
    ...
    if (context == 0) {
      RCUTILS_LOG_ERROR_NAMED("ZenohRouterCheck",
