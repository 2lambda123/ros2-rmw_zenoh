...
saved_msg_data::saved_msg_data(zc_owned_payload_t p, uint64_t recv_ts,
                               const uint8_t pub_gid[16])
...
unsigned int rmw_service_data_t::get_new_uid() { return client_count++; }

void service_data_handler(const z_query_t *query, void *service_data) {
  RCUTILS_LOG_INFO_NAMED("rmw_zenoh_cpp", "[service_data_handler] triggered");
  ...
  // Get the query parameters and payload
  {
    ...
    rmw_service_data->id_query_map.emplace(std::make_pair(
        client_id, std::make_unique<z_owned_query_t>(z_query_clone(query))));
...
//     internal_lock(rmw_client_data->internal_mutex); if
//     (rmw_client_data->condition != nullptr) {
