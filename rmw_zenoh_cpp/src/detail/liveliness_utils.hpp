...
struct NodeInfo {
  ...
  NodeInfo(std::size_t domain_id, std::string ns, std::string name,
           std::string enclave);
};
...
struct TopicInfo {
  ...
  TopicInfo(std::string name, std::string type, std::string qos);
};
...
public:
  ...
  // TODO(Yadunund): Find a way to better bundle the type and the associated
  // data.
  static std::optional<Entity>
  make(EntityType type, NodeInfo node_info,
...
private:
  Entity(EntityType type, NodeInfo node_info,
