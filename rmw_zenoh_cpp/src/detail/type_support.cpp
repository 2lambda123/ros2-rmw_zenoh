...
void TypeSupport::set_members(const message_type_support_callbacks_t *members) {
  ...
  char bounds_info;
  auto data_size =
      static_cast<uint32_t>(members->max_serialized_size(bounds_info));
  max_size_bound_ =
      0 != (bounds_info & ROSIDL_TYPESUPPORT_FASTRTPS_BOUNDED_TYPE);
  is_plain_ = bounds_info == ROSIDL_TYPESUPPORT_FASTRTPS_PLAIN_TYPE;
#else
  is_plain_ = true;
  auto data_size =
      static_cast<uint32_t>(members->max_serialized_size(is_plain_));
...
size_t TypeSupport::get_estimated_serialized_size(const void *ros_message,
...
bool TypeSupport::serialize_ros_message(const void *ros_message,
...
bool TypeSupport::deserialize_ros_message(eprosima::fastcdr::Cdr &deser,
                                          void *ros_message,
                                          const void *impl) const {
  assert(ros_message);
  assert(impl);
  ...
  try {
    ...
    RMW_SET_ERROR_MSG_WITH_FORMAT_STRING(
        "Fast CDR exception deserializing message of type %s.", get_name());
    return false;
  }
