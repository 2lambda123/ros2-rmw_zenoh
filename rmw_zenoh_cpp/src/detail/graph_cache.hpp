...
struct TopicData {
  ...
  TopicData(liveliness::TopicInfo info, TopicStats stats);
};
...
struct GraphNode {
  std::string ns_;
  std::string name_;
  // TODO(Yadunund): Should enclave be the parent to the namespace key and not
  // within a Node?
  std::string enclave_;
...
public:
  ...
  rmw_ret_t
  get_topic_names_and_types(rcutils_allocator_t *allocator, bool no_demangle,
