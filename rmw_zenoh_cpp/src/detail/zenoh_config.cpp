...
/// The name of the default configuration file for the default zenoh session
/// configuration.
...
rmw_ret_t get_z_config(z_owned_config_t *config) {
  ...
  if (NULL != rcutils_get_env(kZenohConfigFileEnvVar, &zenoh_config_path)) {
    ...
    RCUTILS_LOG_ERROR_NAMED("ZenohConfiguration", "Env Var '%s' can't be read.",
                            kZenohConfigFileEnvVar);
    return RMW_RET_ERROR;
  }

  // If the environment variable contains a path to a file, try to read the
  // configuration from it.
  if (zenoh_config_path[0] != '\0') {
    // If the environment variable is set, try to read the configuration from
    // the file.
    *config = zc_config_from_file(zenoh_config_path);
    RCUTILS_LOG_INFO_NAMED(
        "ZenohConfiguration",
        "Using zenoh configuration file pointed by '%s' envar: '%s'",
        kZenohConfigFileEnvVar, zenoh_config_path);
  } else {
    ...
    static const std::string path_to_config_folder =
        ...
        "/config/";
    const std::string default_zconfig_path =
        path_to_config_folder + kDefaultZenohConfigFileName;
    *config = zc_config_from_file(default_zconfig_path.c_str());
    RCUTILS_LOG_INFO_NAMED("ZenohConfiguration",
                           "Using default zenoh configuration file at '%s'",
                           default_zconfig_path.c_str());
  }

  // Verify that the configuration is valid.
  if (!z_config_check(config)) {
    RCUTILS_LOG_ERROR_NAMED("ZenohConfiguration",
                            "Configuration is not valid. Please check the "
