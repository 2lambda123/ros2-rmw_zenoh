...
rmw_node_t *rmw_create_node(rmw_context_t *context, const char *name,
  ...
  RMW_CHECK_TYPE_IDENTIFIERS_MATCH(context, context->implementation_identifier,
                                   rmw_zenoh_identifier, return nullptr);
  ...
  // We specifically don't use rcutils_strdup() here because we want to avoid
  // iterating over the name again looking for the \0 (we already did that
  // above).
  char *new_string =
      static_cast<char *>(allocator->allocate(name_len + 1, allocator->state));
  RMW_CHECK_FOR_NULL_WITH_MSG(
      new_string, "unable to allocate memory for node name", return nullptr);
  ...
  node->data =
      allocator->zero_allocate(1, sizeof(rmw_node_data_t), allocator->state);
  RMW_CHECK_FOR_NULL_WITH_MSG(
      node->data, "unable to allocate memory for node data", return nullptr);
  auto free_node_data = rcpputils::make_scope_exit([node, allocator]() {
  ...
  //   liveliness::GenerateToken::node(context->actual_domain_id, namespace_,
  //   name)
  // );
  // if (!pub_result) {
  //   return nullptr;
  ...
  node_data->token = zc_liveliness_declare_token(
      ...
      z_keyexpr(liveliness_entity->keyexpr().c_str()), NULL);
  auto free_token = rcpputils::make_scope_exit([node]() {
...
rmw_ret_t rmw_destroy_node(rmw_node_t *node) {
  ...
  //   &node->context->impl->session,
  //   liveliness::GenerateToken::node(node->context->actual_domain_id,
  //   node->namespace_,
  //     node->name)
  // );
  // if (!del_result) {
...
// the old string into it. If this becomes a performance problem, we could
// consider modifying the topic_name in place. But this means we need to be much
// more careful about who owns the string.
static z_owned_keyexpr_t
ros_topic_name_to_zenoh_key(const char *const topic_name, size_t domain_id,
  ...
  z_owned_keyexpr_t keyexpr = z_keyexpr_join(z_keyexpr(domain_ss.str().c_str()),
                                             z_keyexpr(stripped_topic_name));
...
find_type_support(const rosidl_message_type_support_t *type_supports) {
  const rosidl_message_type_support_t *type_support =
...
rmw_publisher_t *rmw_create_publisher(
  ...
  //   node, topic_name, &adapted_qos_profile,
  ...
  const rosidl_message_type_support_t *type_support =
      find_type_support(type_supports);
  ...
  // above).
  char *new_string =
      static_cast<char *>(allocator->allocate(topic_len + 1, allocator->state));
  RMW_CHECK_FOR_NULL_WITH_MSG(new_string, "Failed to allocate topic name",
                              return nullptr);
  ...
  // string?
  publisher_data->pub =
      z_declare_publisher(z_loan(context_impl->session), z_loan(keyexpr), NULL);
  ...
  const auto liveliness_entity = liveliness::Entity::make(
      ...
      liveliness::NodeInfo{node->context->actual_domain_id, node->namespace_,
                           node->name, ""},
...
rmw_ret_t rmw_publish(const rmw_publisher_t *publisher, const void *ros_message,
  ...
  bool from_shm = false;
  auto free_msg_bytes =
      rcpputils::make_scope_exit([msg_bytes, allocator, from_shm]() {
  ...
  if (zc_shm_manager_check(&publisher_data->context->impl->shm_manager)) {
    shmbuf = zc_shm_alloc(&publisher_data->context->impl->shm_manager,
                          max_data_length);
    if (!z_check(shmbuf)) {
      ...
      shmbuf = zc_shm_alloc(&publisher_data->context->impl->shm_manager,
                            max_data_length);
      if (!z_check(shmbuf)) {
        // TODO(Yadunund): Should we revert to regular allocation and not return
        // an error?
  ...
  if (!publisher_data->type_support->serialize_ros_message(
          ros_message, ser, publisher_data->type_support_impl)) {
  ...
  if (from_shm) {
    ...
    ret = zc_publisher_put_owned(z_loan(publisher_data->pub), z_move(payload),
                                 &options);
  } else {
...
rmw_ret_t rmw_publish_serialized_message(
  ...
  // Returns 0 if success.
  int8_t ret =
      z_publisher_put(z_loan(publisher_data->pub), serialized_message->buffer,
                      data_length, &options);
...
rmw_subscription_t *rmw_create_subscription(
  ...
  auto destruct_sub_data = rcpputils::make_scope_exit([sub_data]() {
    RMW_TRY_DESTRUCTOR_FROM_WITHIN_FAILURE(sub_data->~rmw_subscription_data_t(),
                                           rmw_subscription_data_t);
  });
  ...
  RMW_TRY_PLACEMENT_NEW(sub_data->type_support, sub_data->type_support,
                        return nullptr, MessageTypeSupport, callbacks);
  auto destruct_msg_type_support = rcpputils::make_scope_exit([sub_data]() {
    RMW_TRY_DESTRUCTOR_FROM_WITHIN_FAILURE(
        sub_data->type_support->~MessageTypeSupport(), MessageTypeSupport);
  });
  ...
  z_owned_closure_sample_t callback =
      z_closure(sub_data_handler, nullptr, sub_data);
  z_owned_keyexpr_t keyexpr = ros_topic_name_to_zenoh_key(
      topic_name, node->context->actual_domain_id, allocator);
  auto always_free_ros_keyexpr = rcpputils::make_scope_exit(
      [&keyexpr]() { z_keyexpr_drop(z_move(keyexpr)); });
  ...
  sub_data->sub =
      z_declare_subscriber(z_loan(context_impl->session), z_loan(keyexpr),
  ...
  auto undeclare_z_sub = rcpputils::make_scope_exit(
      [sub_data]() { z_undeclare_subscriber(z_move(sub_data->sub)); });
...
rmw_ret_t rmw_destroy_subscription(rmw_node_t *node,
  ...
  RMW_CHECK_TYPE_IDENTIFIERS_MATCH(
      ...
      rmw_zenoh_identifier, return RMW_RET_INCORRECT_RMW_IMPLEMENTATION);
...
rmw_subscription_get_actual_qos(const rmw_subscription_t *subscription,
  ...
  qos->reliability = sub_data->reliable
...
static rmw_ret_t __rmw_take(const rmw_subscription_t *subscription,
  ...
  *taken = false;
  ...
  std::unique_ptr<saved_msg_data> msg_data;
  {
    ...
    if (sub_data->message_queue.empty()) {
      // This tells rcl that the check for a new message was done, but no
      // messages have come in yet.
      return RMW_RET_OK;
    }

    // NOTE(CH3): Potential place to handle "QoS" (e.g. could pop from back so
    // it is LIFO)
  ...
  eprosima::fastcdr::FastBuffer fastbuffer(
      reinterpret_cast<char *>(
          const_cast<uint8_t *>(msg_data->payload.payload.start)),
  ...
  if (!sub_data->type_support->deserialize_ros_message(
          deser, ros_message, sub_data->type_support_impl)) {
...
rmw_client_t *rmw_create_client(
  ...
  RCUTILS_LOG_INFO_NAMED("rmw_zenoh_common_cpp",
                         "[rmw_create_client] %s with queue of depth %ld",
                         service_name, qos_profile->depth);
  RMW_CHECK_ARGUMENT_FOR_NULL(node, nullptr);
  ...
  RMW_CHECK_FOR_NULL_WITH_MSG(client_data->request_type_support,
  ...
  RMW_CHECK_FOR_NULL_WITH_MSG(client_data->response_type_support,
...
rmw_ret_t rmw_send_request(const rmw_client_t *client, const void *ros_request,
  ...
  if (!client_data->request_type_support->serialize_ros_message(
          ros_request, ser, client_data->request_type_support_impl)) {
    allocator->deallocate(request_bytes, allocator->state);
    return RMW_RET_ERROR;
  }
  ...
  *sequence_id = 0;
  ...
  opts.value.payload =
      z_bytes_t{data_length, reinterpret_cast<const uint8_t *>(request_bytes)};
  ...
  // client_data->zn_closure_reply = z_closure(client_data_handler, nullptr,
  // client_data);

  // z_get(z_loan(context_impl->session), z_loan(keyexpr), "",
  // &client_data->zn_closure_reply, &opts);

  client_data->channel = zc_reply_non_blocking_fifo_new(16);
  z_get(z_loan(context_impl->session), z_loan(client_data->keyexpr), "",
        z_move(client_data->channel.send),
        &opts); // here, the send is moved and will be dropped by zenoh when
                // adequate
  z_owned_reply_t reply = z_reply_null();
  for (bool call_success = z_call(client_data->channel.recv, &reply);
       !call_success || z_check(reply);
       call_success = z_call(client_data->channel.recv, &reply)) {
    if (!call_success) {
      RCUTILS_LOG_WARN_NAMED("rmw_zenoh_cpp",
                             "[rmw_send_request] call unsuccessful");
      continue;
    }
    if (z_reply_is_ok(&reply)) {
      ...
      RCUTILS_LOG_WARN_NAMED("rmw_zenoh_cpp",
                             "[rmw_send_request] z_reply is not ok");
      return RMW_RET_ERROR;
    }
  }
...
rmw_ret_t rmw_take_response(const rmw_client_t *client,
  ...
  *taken = false;
  ...
  if (client_data->replies.empty()) {
    ...
    RCUTILS_LOG_ERROR_NAMED("rmw_zenoh_cpp",
                            "[rmw_take_response] Response message is empty");
    return RMW_RET_ERROR;
  }
  ...
  if (!client_data->response_type_support->deserialize_ros_message(
          deser, ros_response, client_data->response_type_support_impl)) {
...
rmw_service_t *rmw_create_service(
    const rmw_node_t *node, const rosidl_service_type_support_t *type_supports,
    const char *service_name, const rmw_qos_profile_t *qos_profiles) {
  RCUTILS_LOG_INFO_NAMED("rmw_zenoh_cpp", "[rmw_create_service] %s",
                         service_name);
  ...
  //                        type support as the architecture for the service
  //                        here is different to the one used in DDS (with
  //                        fastcdr).
  auto service_members =
      static_cast<const service_type_support_callbacks_t *>(type_support->data);
  auto request_members = static_cast<const message_type_support_callbacks_t *>(
  ...
  RMW_CHECK_FOR_NULL_WITH_MSG(service_data->request_type_support,
                              "Failed to allocate RequestTypeSupport",
                              return nullptr);
  auto free_request_type_support = rcpputils::make_scope_exit([service_data,
                                                               allocator]() {
  ...
  RMW_CHECK_FOR_NULL_WITH_MSG(service_data->response_type_support,
                              "Failed to allocate ResponseTypeSupport",
                              return nullptr);
  auto free_response_type_support =
      rcpputils::make_scope_exit([service_data, allocator]() {
        allocator->deallocate(service_data->response_type_support,
                              allocator->state);
      });
  ...
  // TODO(francocipollone): Replace ros_topic_name_to_zenoh_key by service
  // related function.
  ...
  RCUTILS_LOG_INFO_NAMED("rmw_zenoh_cpp", "[rmw_create_service] keyexpr: %s",
                         z_loan(z_keyexpr_to_string(z_loan(keyexpr))));

  z_owned_closure_query_t callback =
      z_closure(service_data_handler, nullptr, service_data);

  service_data->qable =
      z_declare_queryable(z_loan(context_impl->session), z_loan(keyexpr),
  ...
  *taken = false;
  ...
  if (service_data->id_query_map.empty()) {
    ...
    RCUTILS_LOG_INFO_NAMED("rmw_zenoh_cpp",
                           "[rmw_take_request] Take id_query_map is empty");
    return RMW_RET_OK;
  }
  ...
  if (!z_query_check(owned_query_ptr)) {
    RMW_SET_ERROR_MSG(
        "onwed_query_t contains gravestone, can't deserialize message");
    return RMW_RET_ERROR;
  }
  ...
  eprosima::fastcdr::FastBuffer fastbuffer(
      reinterpret_cast<char *>(
          const_cast<uint8_t *>(payload_value.payload.start)),
  ...
  if (!service_data->request_type_support->deserialize_ros_message(
          deser, ros_request, service_data->request_type_support_impl)) {
    RMW_SET_ERROR_MSG("could not deserialize ROS message");
    return RMW_RET_ERROR;
  }

  RCUTILS_LOG_INFO_NAMED("rmw_zenoh_cpp",
                         "[rmw_take_request] deserialized message");
...
rmw_ret_t rmw_send_response(const rmw_service_t *service,
  ...
  if (!service_data->response_type_support->serialize_ros_message(
          ros_response, ser, service_data->response_type_support_impl)) {
    allocator->deallocate(response_bytes, allocator->state);
    return RMW_RET_ERROR;
  }
  ...
  auto owned_query_ptr =
      service_data->id_query_map[request_header->sequence_number].get();
  service_data->query_queue_mutex.unlock();
  ...
  z_query_reply(query_ptr, z_query_keyexpr(query_ptr),
                reinterpret_cast<const uint8_t *>(response_bytes),
                data_length + meta_length, &options);
...
rmw_guard_condition_t *rmw_create_guard_condition(rmw_context_t *context) {
  ...
  auto destruct_guard_condition =
      rcpputils::make_scope_exit([guard_condition]() {
        RMW_TRY_DESTRUCTOR_FROM_WITHIN_FAILURE(
            static_cast<GuardCondition *>(guard_condition->data)
                ->~GuardCondition(),
            GuardCondition);
      });
...
rmw_wait_set_t *rmw_create_wait_set(rmw_context_t *context,
  ...
  wait_set->data = allocator->zero_allocate(1, sizeof(rmw_wait_set_data_t),
                                            allocator->state);
  RMW_CHECK_FOR_NULL_WITH_MSG(
  ...
  auto destruct_rmw_wait_set_data = rcpputils::make_scope_exit([wait_set]() {
    RMW_TRY_DESTRUCTOR_FROM_WITHIN_FAILURE(
        static_cast<rmw_wait_set_data_t *>(wait_set->data)
            ->~rmw_wait_set_data_t(),
        rmw_wait_set_data);
  });
...
rmw_ret_t rmw_destroy_wait_set(rmw_wait_set_t *wait_set) {
  ...
  RMW_CHECK_TYPE_IDENTIFIERS_MATCH(
      wait_set, wait_set->implementation_identifier, rmw_zenoh_identifier,
      return RMW_RET_INCORRECT_RMW_IMPLEMENTATION);
...
rmw_ret_t rmw_wait(rmw_subscriptions_t *subscriptions,
  ...
  RCUTILS_LOG_WARN_NAMED("rmw_zenoh_cpp",
                         "[rmw_wait] %ld subscriptions, %ld services, %ld "
                         "clients, %ld events, %ld guard conditions",
                         subscriptions->subscriber_count,
                         services->service_count, clients->client_count,
                         events->event_count,
                         guard_conditions->guard_condition_count);

  // TODO(yadunund): Switch to debug log level.
  if (wait_timeout) {
    RCUTILS_LOG_WARN_NAMED("rmw_zenoh_common_cpp",
                           "[rmw_wait] TIMEOUT: %ld s %ld ns",
                           wait_timeout->sec, wait_timeout->nsec);
  }
  ...
  if (guard_conditions) {
    // Go through each of the guard conditions, and attach the wait set
    // condition variable to them. That way they can wake it up if they are
    // triggered while we are waiting.
    for (size_t i = 0; i < guard_conditions->guard_condition_count; ++i) {
      // This is hard to track down, but each of the (void *) pointers in
      // guard_conditions->guard_conditions points to the data field of the
      // related rmw_guard_condition_t.  So we can directly cast it to
      // GuardCondition.
      GuardCondition *gc =
          static_cast<GuardCondition *>(guard_conditions->guard_conditions[i]);
      if (gc != nullptr) {
  ...
  if (subscriptions) {
    // Go through each of the subscriptions and attach the wait set condition
    // variable to them. That way they can wake it up if they are triggered
    // while we are waiting.
    for (size_t i = 0; i < subscriptions->subscriber_count; ++i) {
      auto sub_data =
          static_cast<rmw_subscription_data_t *>(subscriptions->subscribers[i]);
      if (sub_data != nullptr) {
  ...
  if (services) {
    // Go through each of the services and attach the wait set condition
    // variable to them. That way they can wake it up if they are triggered
    // while we are waiting.
  ...
  if (clients) {
    // Go through each of the clients and attach the wait set condition variable
    // to them. That way they can wake it up if they are triggered while we are
    // waiting.
    for (size_t i = 0; i < clients->client_count; ++i) {
      rmw_client_data_t *client_data =
          static_cast<rmw_client_data_t *>(clients->clients[i]);
  ...
  // "wait forever", if it specified by 0 it means "never wait", and if it is
  ...
  if (wait_timeout == nullptr) {
    ...
    if (wait_timeout->sec != 0 || wait_timeout->nsec != 0) {
      wait_set_data->condition_variable.wait_for(
          lock, std::chrono::nanoseconds(wait_timeout->nsec +
                                         RCUTILS_S_TO_NS(wait_timeout->sec)));
    }
  }

  if (guard_conditions) {
    // Now detach the condition variable and mutex from each of the guard
    // conditions
    for (size_t i = 0; i < guard_conditions->guard_condition_count; ++i) {
      GuardCondition *gc =
          static_cast<GuardCondition *>(guard_conditions->guard_conditions[i]);
  ...
  if (subscriptions) {
    // Now detach the condition variable and mutex from each of the
    // subscriptions
    for (size_t i = 0; i < subscriptions->subscriber_count; ++i) {
      auto sub_data =
          static_cast<rmw_subscription_data_t *>(subscriptions->subscribers[i]);
      if (sub_data != nullptr) {
        ...
        // array that have *not* been triggered should be set to NULL
        if (sub_data->message_queue.empty()) {
          // Setting to nullptr lets rcl know that this subscription is not
          // ready
