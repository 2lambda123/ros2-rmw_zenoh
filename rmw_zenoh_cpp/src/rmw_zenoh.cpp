...
rmw_ret_t rmw_destroy_node(rmw_node_t *node) {
  ...
  const bool result =
      PublishToken::del(&node->context->impl->session,
                        GenerateToken::node(node->context->actual_domain_id,
...
rmw_publisher_t *rmw_create_publisher(
  ...
  publisher_data->graph_cache_handle =
      node->context->impl->graph_cache.add_publisher(
          rmw_publisher->topic_name, node->name, node->namespace_,
          publisher_data->type_support->get_name(), allocator);
  auto remove_from_graph_cache =
      rcpputils::make_scope_exit([node, publisher_data]() {
        node->context->impl->graph_cache.remove_publisher(
            publisher_data->graph_cache_handle);
      });
...
rmw_subscription_t *rmw_create_subscription(
  ...
  sub_data->graph_cache_handle =
      node->context->impl->graph_cache.add_subscription(
          rmw_subscription->topic_name, node->name, node->namespace_,
          sub_data->type_support->get_name(), allocator);
  auto remove_from_graph_cache = rcpputils::make_scope_exit([node, sub_data]() {
    node->context->impl->graph_cache.remove_subscription(
        sub_data->graph_cache_handle);
  });
