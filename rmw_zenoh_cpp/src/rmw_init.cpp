...
rmw_ret_t rmw_init(const rmw_init_options_t *options, rmw_context_t *context) {
  ...
  const std::string liveliness_str =
      liveliness::subscription_token(context->actual_domain_id);
  ...
  zc_liveliness_get(z_loan(context->impl->session),
                    z_keyexpr(liveliness_str.c_str()), z_move(channel.send),
                    NULL);
  ...
  //   z_move(channel.send), &opts);      // here, the send is moved and will be
  //   dropped by zenoh when adequate
