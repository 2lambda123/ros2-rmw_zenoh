...
rmw_ret_t rmw_init(const rmw_init_options_t *options, rmw_context_t *context) {
  ...
  z_owned_str_t shm_enabled =
      zc_config_get(z_loan(config), "transport/shared_memory/enabled");
  auto free_shm_enabled = rcpputils::make_scope_exit(
      [&shm_enabled]() { z_drop(z_move(shm_enabled)); });
  ...
  if ((ret = zenoh_router_check(z_loan(context->impl->session))) !=
      RMW_RET_OK) {
  ...
  if (shm_enabled._cstr != nullptr && strcmp(shm_enabled._cstr, "true") == 0) {
    z_id_t id = z_info_zid(z_loan(context->impl->session));
    char idstr[sizeof(id.id) * 2 +
               1]; // 2 bytes for each byte of the id, plus the trailing \0
    static constexpr size_t max_size_of_each =
        3; // 2 for each byte, plus the trailing \0
    ...
    // though it's not a standard parameter?
    context->impl->shm_manager =
        zc_shm_manager_new(z_loan(context->impl->session), idstr,
  ...
  auto free_shm_manager = rcpputils::make_scope_exit(
      [context]() { z_drop(z_move(context->impl->shm_manager)); });
  ...
  RMW_TRY_PLACEMENT_NEW(context->impl->graph_guard_condition->data,
                        context->impl->graph_guard_condition->data,
                        return RMW_RET_BAD_ALLOC, GuardCondition);
  auto destruct_guard_condition_data = rcpputils::make_scope_exit([context]() {
    auto gc_data = static_cast<GuardCondition *>(
        context->impl->graph_guard_condition->data);
  ...
  const std::string liveliness_str =
      GenerateToken::liveliness(context->actual_domain_id);

  // Query the router to get graph information before this session was started.
  // TODO(Yadunund): This will not be needed once the zenoh-c liveliness API is
  // available.
  RCUTILS_LOG_WARN_NAMED(
      "rmw_zenoh_cpp",
  ...
  z_get(z_loan(context->impl->session), z_keyexpr(liveliness_str.c_str()), "",
        z_move(channel.send),
  ...
  z_owned_closure_sample_t callback =
      z_closure(graph_sub_data_handler, nullptr, context->impl);
...
rmw_ret_t rmw_context_fini(rmw_context_t *context) {
  ...
  // rmw_shutdown() is invoked before rmw_destroy_node() however we still need
  // the session alive for the latter.
  ...
  RMW_TRY_DESTRUCTOR(
      static_cast<GuardCondition *>(context->impl->graph_guard_condition->data)
          ->~GuardCondition(),
      GuardCondition, );
  allocator->deallocate(context->impl->graph_guard_condition->data,
                        allocator->state);
