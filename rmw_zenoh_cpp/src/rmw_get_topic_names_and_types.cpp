...
rmw_get_topic_names_and_types(const rmw_node_t *node,
  ...
  auto cleanup_names_and_types =
      rcpputils::make_scope_exit([topic_names_and_types] {
  ...
  auto free_names =
      rcpputils::make_scope_exit([topic_names_and_types, allocator] {
        allocator->deallocate(topic_names_and_types->names.data[0],
                              allocator->state);
      });
  ...
  auto fini_string_array = rcpputils::make_scope_exit([topic_names_and_types] {
    rmw_ret_t fail_ret =
        rcutils_string_array_fini(topic_names_and_types->types);
    if (fail_ret != RMW_RET_OK) {
      RMW_SAFE_FWRITE_TO_STDERR(
          "failed to cleanup topic types during error handling");
    }
  });

  topic_names_and_types->types[0].data[0] =
      rcutils_strdup("std_msgs/msg/String", *allocator);
  ...
  auto free_types =
      rcpputils::make_scope_exit([topic_names_and_types, allocator] {
        allocator->deallocate(topic_names_and_types->types[0].data[0],
                              allocator->state);
      });
